		...
		if list(this_table_data.columns).count('Season') > 0:
			...
			for idx, row in this_table_data.iterrows():
				...
				if team_name[-1] == '*':
					...
					playoff_bid = False
				team_division = row['Division'][0].strip()
				team_conference = row['Conference'][0].strip()
				# Store the data about the franchise that season into a data structure and append it to the franchise history list
				seasons_data.append({'FranchiseID': franchise_id, 'FranchiseName': franchise_name, 'LeagueID': league_id, 'TeamName': team_name, 'Season': season_year, 'SeasonName': season_name, 'TeamID': team_id, 'InPlayoffs': playoff_bid, 'Division': team_division, 'Conference': team_conference})
	# Return the franchise history list
	return seasons_data
...
def main ():
	...
	# Get the list of franchises by requesting the page with all franchises
	franchises_url = 'https://www.hockey-reference.com/teams/'
	...
	if franchises_page is None:
		...
		franchise_data = []
		# Iterate through the tables on the franchise page
		for html_table in franchises_data_tables:
			franchises_list = parse_sref_franchises_table(html_table)
			# Retrive history data for each franchise in the table and parse it
			for franchise_id in list(franchises_list.keys()):
				print(franchise_id)
				franchise_name = franchises_list[franchise_id]
				# This is a workaround specific to PHX, which does not currently have a history.html; I may try to modify this in the future to detect this automatically instead of having this workaround
				if franchise_id == 'PHX':
					cur_franchise_url = (('https://www.hockey-reference.com/teams/%s/') % (franchise_id))
				else:
					cur_franchise_url = (('https://www.hockey-reference.com/teams/%s/history.html') % (franchise_id))
